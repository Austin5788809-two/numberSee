...
struct network
{
    ...
    std::vector<std::function<double(double)>> act_f;     // act_f[i]       表示第i层使用的激活函数
    std::vector<std::function<double(double)>> act_f_d;   // act_f[i]       表示第i层使用的激活函数的导数
    ...
    network(size_t lnum, const std::vector<size_t>& nnum);
    void forward(const std::vector<double>& input, std::vector<double>& output, std::vector<double>& prob);           // 前向传播, 返回输出层的值，用于测试
